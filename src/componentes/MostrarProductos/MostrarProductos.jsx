...
function TarjetaProducto({ p, categorias, mostrarMensaje }) {
  ...
  const categoria = categorias.find((c) => c.id === (p.categoria?.id || p.categoria_id || p.categoriaId));
  ...
  if (cargando) {
    return (
      <div className="container text-center my-5 py-5">
        ...
        <h5 className="mt-3 text-muted">Cargando catálogo...</h5>
      </div>
    );
  }
      ...
      <div className="row g-4 justify-content-center">
        {productos.length === 0 ? (
          <div className="col-12 text-center py-5">
            <h5 className="text-muted">No hay productos disponibles en esta sección.</h5>
          </div>
        ) : (
          productos.map((p) => (
            <TarjetaProducto
